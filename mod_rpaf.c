...
static int is_in_array(apr_sockaddr_t *client_addr, apr_array_header_t *proxy_ips) {
    int i;
    apr_ipsubnet_t **subs = (apr_ipsubnet_t **)proxy_ips->elts;

    for (i = 0; i < proxy_ips->nelts; i++) {
        if (apr_ipsubnet_test(subs[i], client_addr)) {
            return 1;
        }
    }
...
static char *last_not_in_array(request_rec *r, apr_array_header_t *forwarded_for,
    ...
    proxy_list = apr_pstrdup(r->pool, r->connection->client_ip);
